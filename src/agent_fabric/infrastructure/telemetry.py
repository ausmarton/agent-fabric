"""Optional OpenTelemetry tracing for agent-fabric.

When ``opentelemetry-api`` and ``opentelemetry-sdk`` are installed (optional
dependency: ``pip install "agent-fabric[otel]"``) this module sets up a tracer
...
Configuration (``FabricConfig.telemetry``)::
...
def setup_telemetry(config: "FabricConfig") -> None:
    ...
    if not _otel_available:
        logger.warning(
            ...
            "Install with: pip install 'agent-fabric[otel]'"
        )
        return
...
def _setup_otel_tracer(tel_cfg: Any) -> None:
    ...
    _tracer = trace.get_tracer("agent_fabric")
